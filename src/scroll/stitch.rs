...
pub fn find_scroll_delta(a: &CapturedFrame, b: &CapturedFrame) -> Option<usize> {
    ...
    let cols = sample_cols(w);
    let sig_a = row_signatures(a, w, h, &cols);
    let sig_b = row_signatures(b, w, h, &cols);
    ...
    let mut candidates: Vec<(u64, usize)> = Vec::with_capacity(max_s);
    for s in 1..=max_s {
        // 比较 a[s..h] 与 b[0..h-s]
        let mut score = 0u64;
        let n = h - s;
        for r in 0..n {
            score += sig_a[s + r].abs_diff(sig_b[r]);
        }
        let avg = score / n as u64;
        candidates.push((avg, s));
    }
    // 分数升序，取前几个候选逐个做像素验证
    candidates.sort_unstable_by_key(|(avg, _)| *avg);
    for (avg, s) in candidates.into_iter().take(3) {
        if avg == u64::MAX {
            break;
        }
        if band_has_energy(&sig_a, s, h) && verify_rows(a, b, s, w) {
            return Some(s);
        }
    }
    None
...
fn sample_cols(w: usize) -> Vec<usize> {
    let usable_w = w.saturating_sub(w / SKIP_RIGHT_RATIO).max(1);
    let k = SAMPLE_COLS.min(usable_w);
    (0..k).map(|i| usable_w * i / k.max(1)).collect()
}

/// 每行签名 = 采样列 RGB 之和（丢弃 alpha，通常恒为 255）
fn row_signatures(f: &CapturedFrame, w: usize, h: usize, cols: &[usize]) -> Vec<u64> {
    let px = &f.pixels;
    let mut sig = Vec::with_capacity(h);
    for r in 0..h {
        let mut acc = 0u64;
        for &c in cols {
            let p = (r * w + c) * 4;
...
fn verify_rows(a: &CapturedFrame, b: &CapturedFrame, s: usize, w: usize) -> bool {
    let h = a.height as usize;
    let n = h - s; // 重叠行数
    if n < 5 {
        return false;
    }
    let vcols: Vec<usize> = (0..8).map(|i| w * (i + 1) / 9).collect();
