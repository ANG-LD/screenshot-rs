...
impl Engine {
    ...
    fn beam_search(&mut self, enc: &Encoded) -> Result<Vec<i64>, String> {
        ...
        let hidden = Tensor::from_array(([1usize, s, d], enc.data.clone()))
        ...
        for _ in 0..max_new {
            ...
            for (bi, beam) in active.iter().enumerate() {
                let prev = beam.ids.last().copied().unwrap_or(self.cfg.decoder_start_id);
                let logits = self.step_logits(&hidden, &mask, &beam.ids)?;
                // 三条约束，全在**原始 logit** 上做（见 REP_PENALTY 的注释说明为什么）：
                //   ① pad 封禁 —— 照模型 generation_config 的 bad_words_ids=[[65000]]；
                //   ② 相邻二元组不重复 —— 治「截截图」这类退化；
                //   ③ 近期出现过的 token 惩罚 logit/1.15 —— 治长距离重复。
                let mut adj = logits;
                let repeated: Vec<bool> = (0..adj.len())
                    .map(|i| beam.ids.iter().rev().take(REP_WINDOW).any(|t| *t == i as i64))
                    .collect();
                for (i, v) in adj.iter_mut().enumerate() {
                    let tok = i as i64;
                    if tok == self.cfg.pad_id
                        || prev == tok
                        || beam.ids.windows(2).any(|w| w[0] == prev && w[1] == tok)
                    {
                        *v = f32::NEG_INFINITY;
                    } else if repeated[i] {
                        *v /= REP_PENALTY;
                ...
                // log_softmax 单调，所以按调整后的 logit 排序 == 按对数概率排序；
                // 分数本身仍用对数概率累加（beam 长度归一化要用它）。
                let logp = log_softmax(&adj);
                ...
                for (lp, tok) in scored.into_iter().take(k) {
    ...
    /// 跑一次解码器前向，返回**最后一个位置**的原始 logit 分布。
    ///
    /// 故意返回原始 logit 而不是 log-softmax：重复惩罚必须作用在 logit 上才温和
    /// （见 `REP_PENALTY` 的注释）。
    fn step_logits(
        &mut self,
        hidden: &Tensor<f32>,
        ...
        Ok(data[base..base + vocab].to_vec())
    }
}
